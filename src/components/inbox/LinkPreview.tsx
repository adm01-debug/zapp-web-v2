...
export function LinkPreview({ url, className, compact = false, showRemove, onRemove }: LinkPreviewProps) {
  ...
  if (metadata.type === 'image' && metadata.image && !imageError) return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={cn("relative group rounded-xl overflow-hidden", className)}>
      ...
      <a href={url} target="_blank" rel="noopener noreferrer"><img src={metadata.image} alt={metadata.title || 'Image'} onError={() = loading="lazy" decoding="async"> setImageError(true)} className="max-w-full max-h-64 rounded-xl object-cover hover:scale-[1.02] transition-transform" /></a>
    </motion.div>
  );
  ...
  if (compact) return (
    <motion.a href={url} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }}
      ...
      {metadata.favicon && !imageError ? <img src={metadata.favicon} alt="" className="w-4 h-4 rounded" onError={() = loading="lazy" decoding="async"> setImageError(true)} /> : <Globe className="w-4 h-4 text-muted-foreground" />}
    ...
    <motion.a href={url} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      {metadata.image && !imageError && (
        <div className="relative aspect-video bg-muted overflow-hidden">
          <img src={metadata.image} alt={metadata.title || ''} onError={() = loading="lazy" decoding="async"> setImageError(true)} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
