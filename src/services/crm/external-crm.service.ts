...
export class ExternalCRMService {
  static async getContact360Batch(phones: string[]): Promise<Map<string, CRMBatchResult>> {
    ...
    if (data && typeof data === 'object') {
      for (const [phone, info] of Object.entries(data as Record<string, CRMBatchResult>)) {
        map.set(phone, info);
  ...
  static async fetchUniqueCompanies(maxPages = 5, pageSize = 200) {
    ...
    while (page < maxPages) {
      ...
      const results = (data as AdvancedSearchResponse | null)?.results || [];
  ...
  static async fetchUniqueRoles() {
    ...
    if (!e1 && salesRoles) {
      (salesRoles as Array<{ role: string | null }>).forEach((r) => {
    ...
    if (!e2 && searchData) {
      const results = (searchData as AdvancedSearchResponse | null)?.results || [];
      results.forEach((r) => {
